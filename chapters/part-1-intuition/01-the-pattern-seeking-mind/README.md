...
|--------|------|-------------|
| 1.1 | `src/figures/1.1.png` | Random Points in a Square (clustering illusion) |
| 1.2 | `src/figures/1.2.png` | Distribution of Longest Streaks (100 coin flips, 10k simulations) |
| 1.3 | `src/figures/1.3.png` | The Birthday Paradox (probability curve) |
| 1.4 | `src/figures/1.4.png` | Clustering in a Grid |
| 1.5 | `src/figures/1.5.png` | Hot Hand vs Random |
...
- Automatically applies SciencePlots styling
- Saves with consistent 300 DPI resolution
- Exports to the local `src/figures/` directory

To add a new figure:

1. Create a `generate_figure_1_X()` function in [src/generate_figures.py](src/generate_figures.py)
2. Create a figure using `fig = plt.figure(figsize=(...))` 
3. Plot your content using `fig.add_subplot()` or direct matplotlib calls
4. Call `save_figure(fig, "1.X.png")` to save
5. Call the function from `generate_all_figures()`
