...
This chapter uses a centralized figure management system. All figures are generated programmatically and exported to `src/figures/` with consistent naming and styling.

### Quick Start

To generate all figures for this chapter:
...
This will:
- Generate 3 publication-quality figures (PNG format, 300 DPI)
- Save them to `src/figures/` with names: `1.1.1.png`, `1.1.2.png`, `1.1.3.png`
- Apply consistent SciencePlots styling (science style) with optimal margins

### Figure Index

| Figure | File | Description |
|--------|------|-------------|
| 1.1.1 | `src/figures/1.1.1.png` | Random Points in a Square (clustering illusion) |
| 1.1.2 | `src/figures/1.1.2.png` | Distribution of Longest Streaks (100 coin flips, 10k simulations) |
| 1.1.3 | `src/figures/1.1.3.png` | The Birthday Paradox (probability curve) |

### How to Add New Figures

Each figure is generated by a function in [src/generate_figures.py](src/generate_figures.py) following this pattern:

```python
def generate_figure_1_1():
    """Figure 1.1.1: Description"""
...
The `figure()` context manager:
- Automatically creates a figure with SciencePlots styling
- Handles naming as `{part}.{chapter}.{figure_num}.png`
- Applies margin padding (configurable per-figure)
- Exports to the local `src/figures/` directory with 300 DPI
- Cleans up resources automatically

To add a new figure:

1. Create a `generate_figure_1_X()` function in [src/generate_figures.py](src/generate_figures.py)
2. Use `with figure(1, 1, X, output_dir=OUTPUT_DIR) as fig:` to create the figure
3. Plot your content using `fig.add_subplot()` or direct matplotlib calls
4. Call the function from `generate_all_figures()`

### Configuration

**Global defaults** (in [shared/figure_config.py](../../../shared/figure_config.py)):
- Style: `science` (from SciencePlots)
- Margin pad: `1.5` (tight_layout padding)
- Default figsize: `(10, 6)`

**Per-figure overrides:**

```python
with figure(1, 1, 4, figsize=(12, 8), margin_pad=2.0, output_dir=OUTPUT_DIR) as fig:
    # ... plot code ...
```
...
<img src="./src/figures/1.1.1.png" alt="Clustering Illusion: Truly Random vs Human Random" width="600">

**Figure 1.1.1:** Random points in a square. Notice the visible clusters and gaps—this is what truly random data looks like. Our brains interpret these clusters as suspicious, but they're exactly what statistical randomness produces.
...
<img src="./src/figures/1.1.2.png" alt="Distribution of longest streaks in 100 coin flips" width="600">

**Figure 1.1.2:** Distribution of longest streaks in 100 coin flips across 10,000 simulations. The mean is approximately 7, and streaks of 10 or more occur regularly. This distribution captures the clustering illusion: randomness naturally produces the "rare" patterns we expect not to see.
...
<img src="./src/figures/1.1.3.png" alt="The Birthday Paradox" width="600">

**Figure 1.1.3:** The Birthday Paradox: probability of at least two shared birthdays increases rapidly with group size. The transition around 23 people is surprisingly sharp, showing how many opportunities for coincidence make them nearly inevitable.
